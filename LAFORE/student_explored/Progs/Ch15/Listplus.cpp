...
int main()
   ...
   for(j=0; j<5; j++)
      ...
      list<int>::iterator iter4 = list2.end();
      list<int>::iterator iter5 = dest.begin();
merge(iter1,iter2,iter3,iter4,dest.begin());
while( !dest.empty())
      {
      cout << dest.front() << ' ';  // read item from front
      dest.pop_front();             // pop item off front
      }
  ...
  //how to merge lists  with the algotirm
  //checkkkk

...
int main()
   ...
   person searPerson(searchLastName, searchFirstName, 0);

                             // get count of such persons
   int cntPersons = persSet.count(searPerson);
   cout << "Number of persons with this name = " << cntPersons;

                             // display all matches
   iter = persSet.lower_bound(searPerson);
   while( iter != persSet.upper_bound(searPerson) )
      (*iter++).display();
   cout << endl;
   return 0;
   }  // end main()
//THIS STL REQUIRES FEW COMMON FNCTIONS TO WORK WITH...
//OVERLOADED ==,< ,=,COPY CONSTRUCTOR,DESTRUCTOR IN THIS ONLY WE TAKE FOR SIMPLICITY

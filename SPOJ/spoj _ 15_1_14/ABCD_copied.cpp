...
int main()
{
    ...
    int abcd[4] = {0};
    for (int i = 0; i < n; ++i) {
        abcd[niz[i]-'A']++;//occurences of each stored in the array  at 0, 1, 2, 3
    }
    ...
    for (int i = 0; i < n; ++i) {
        ...
        for (int j = 0; j < 4; ++j) {
            if (abcd[j] == limit || (i > 0 && novi_niz[i-1] == j+'A') || niz[i] == j+'A') continue;// checking for indexes if the adjacent are same
